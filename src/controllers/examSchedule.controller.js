...
const getScheduleByTeacher = async (req, res) => {
  try {
    ...
    const filteredSchedules = schedules.filter(schedule => {
      if (!schedule.subjectId) return false;
      if (schedule.subjectId.isActive === false) return false;
    ...
    const formatted = paginatedSchedules.map(schedule => {
      const classInfo = schedule.classId ? {
        _id: schedule.classId._id,
        name: schedule.classId.class || schedule.classId.name || 'Unknown Class',
      } : null;

      const sectionInfo = schedule.classId ? extractSection(schedule.classId, schedule.sectionId) : null;

      const subjectInfo = schedule.subjectId ? {
        _id: schedule.subjectId._id,
        name: schedule.subjectId.name || 'Unknown Subject',
        code: schedule.subjectId.code || 'N/A',
      } : null;

      const teacherInfo = schedule.teacherId ? {
        _id: schedule.teacherId._id,
        name: schedule.teacherId.name || 'Unknown Teacher',
        email: schedule.teacherId.email || 'N/A',
        role: schedule.teacherId.role || 'teacher'
      } : null;
...
// const extractSection = (classObj, sectionId) => {
//   if (!classObj || !classObj.sections) return null;
//   const sec = classObj?.sections?.find(
//     (s) => s._id.toString() === sectionId?.toString()
//   );
//   return sec ? { _id: sec._id, name: sec.name } : null;
...
//     const notificationData = {
//       school: schoolId,
//       title: `Exam Schedule - ${examData.type?.toUpperCase() || 'EXAM'}`,
//       message: `${examData.type?.toUpperCase() || 'EXAM'} Exam \n` +
//         `For ${examData.subjectName || 'Unknown Subject'}` +
//         ` on ${examData.examDate ? examData.examDate.toDateString() : 'TBD'} ` +
//         `at ${examData.startTime || 'TBD'} - ${examData.endTime || 'TBD'} ` +
//         `in class ${classSection.class} and section ${sectionName} \n` +
//         `Regards ${examData.teacherName || 'To be announced'}`,
//       createdBy: createdBy,
//       category: 'notice',
...
//   } catch (error) {
//     console.error('Error creating exam notification:', error.message);
//     return null;
//   }
// };
...
//     const notificationData = {
//       school: schoolId,
//       title: `Exam Updated - ${examData.type?.toUpperCase() || 'EXAM'}`,
//       message: `${examData.type?.toUpperCase() || 'EXAM'} Exam Updated\n` +
//         `For ${examData.subjectName || 'Unknown Subject'}` +
//         ` on ${examData.examDate ? examData.examDate.toDateString() : 'TBD'}` +
//         ` at ${examData.startTime || 'TBD'} - ${examData.endTime || 'TBD'}\n` +
//         ` in Class: ${classSection.class} and section ${sectionName}\n` +
...
//     const notificationData = {
//       school: schoolId,
//       title: `Exam Cancelled - ${examData.type?.toUpperCase() || 'EXAM'}`,
//       message: `EXAM CANCELLED\n` +
//         `For Subject: ${examData.subjectName || 'Unknown Subject'}` +
//         ` on ${examData.examDate ? examData.examDate.toDateString() : 'TBD'}\n` +
//         ` at ${examData.startTime || 'TBD'} - ${examData.endTime || 'TBD'}` +
//         ` in class ${classSection.class} and section ${sectionName}` +
//         reasonMessage,
...
//   } catch (error) {
//     console.error('Error creating exam cancellation notification:', error.message);
//     return null;
//   }
// };
...
//     // Filter out schedules with null or inactive subjects and valid classId
//     const filteredSchedules = schedules.filter(schedule => {
//       if (!schedule.subjectId) return false;
//       if (schedule.subjectId.isActive === false) return false;
//       if (!schedule.classId) return false;
...
//     // Handle both old and new student schema
//     const classId = student.classId || (student.classInfo && student.classInfo.id);
//     const sectionId = student.sectionId || (student.sectionInfo && student.sectionInfo.id);
//     const schoolId = student.school;
...
//   if (!examStart || !examEnd) return null;
...
//         return {
//           success: false,
//           message: `Class already has "${conflict.subjectId?.name}" exam from ${conflict.startTime} to ${conflict.endTime} on ${examDateTime?.toDateString()}`
//         };
//       }
//     }
//   }
