...
// Helper function to find user in both models
const findUserById = async (id) => {
    let user = await Staff.findById(id).select("-password");
    if (user) return user;
...
const protect = async (req, res, next) => {
    try {
        ...
        // Check user's own token version if exists
...
const checkAllPermissions = (...permissions) => {
    return (req, res, next) => {
        ...
        if (missing.length > 0) {
            return res.status(403).json({
                success: false,
                message: `Access denied. Missing: ${missing.join(', ')}`
            });
        }
